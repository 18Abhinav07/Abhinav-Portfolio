...
export default async function EpisodePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const accentText = e.accent === "primary" ? "text-primary" : "text-secondary";
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-column-gap">
          <div className="md:col-span-3 hidden md:block">
            <div className="sticky top-[120px] font-mono text-label-mono uppercase tracking-[0.18em] text-on-surface-variant">
              <div className={accentText}>Episode {e.index}</div>
              <div className="mt-stack-sm">{e.date}</div>
            </div>
          </div>
          ...
          <div className="md:col-span-9 max-w-3xl">
            {e.body.map((b, i) => {
              ...
              if (b.kind === "phase") {
                return (
                  <Reveal key={i} y={16}
                    className={`font-mono text-label-mono uppercase tracking-[0.18em] ${accentText} mt-stack-xl mb-stack-md brutalist-rule-t pt-stack-md`}
                  >
                    {b.text}
                  </Reveal>
                );
